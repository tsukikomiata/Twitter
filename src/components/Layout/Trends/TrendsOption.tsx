...
function TrendsOption({type, title, countTweets}: {type?: any, title?: any, countTweets?: number}) {
    return (
        <div className={styles.trendsOption}>
            <div className={styles.trendsOption_info}>
                ...
                <p className={styles.trendsOption_info__countTweets}>{countTweets?.toString() + ' Tweets'}</p>
            </div>
            {optionIcon}
        </div>
    )
